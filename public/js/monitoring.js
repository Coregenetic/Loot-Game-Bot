// ─── Monitoring Tab — Echtzeit-Systemmetriken via WebSocket ──────────────────
let monChart = null;
let monWs    = null;
const MON_WINDOW = 60; // 60 Datenpunkte × 2s = 2 Minuten
...
function _monConnectWs() {
  ...
  monWs.onopen = () => {
    ...
    if (token) monWs.send(JSON.stringify({ type: 'auth', token }));
  };

  monWs.onmessage = (e) => {
    try {
      const msg = JSON.parse(e.data);
      if (msg.type === 'server_metrics') _monApply(msg);
    } catch {}
  };
...
function _monApply(m) {
  ...
  if (m.cpu !== null && m.cpu !== undefined) {
    ...
    setStyle('mon-cpu-val', 'color', cpuColor);

    const canvas = document.getElementById('mon-cpu-chart');
    if (canvas) {
      if (!monChart) {
      ...
      if (monChart.data.labels.length > MON_WINDOW) {
        monChart.data.labels.shift();
        monChart.data.datasets[0].data.shift();
      }

      // Farbe dynamisch anpassen
      ...
      const bg    = avg > 80 ? 'rgba(248,113,113,0.08)' : avg > 50 ? 'rgba(251,191,36,0.08)' : 'rgba(16,185,129,0.08)';
      monChart.data.datasets[0].borderColor = color;
      monChart.data.datasets[0].backgroundColor = bg;
      set('mon-cpu-graph-val', m.cpu.toFixed(1) + '%');
      document.getElementById('mon-cpu-graph-val').style.color = color;
  ...
  if (m.ram) {
    ...
    set('mon-ram-val', usedGb + '<span style="font-size:16px">GB</span>');
    document.getElementById('mon-ram-val').innerHTML = usedGb + '<span style="font-size:16px;color:#475569"> GB</span>';
    ...
    if (ramBar) {
      ramBar.style.width = m.ram.pct + '%';
      ramBar.style.background = m.ram.pct > 80 ? '#f87171' : '#60a5fa';
    }

    // Heap
    set('mon-heap-val', m.heap?.usedMb + '<span style="font-size:16px;color:#475569"> MB</span>');
    document.getElementById('mon-heap-val').innerHTML = (m.heap?.usedMb || '—') + '<span style="font-size:16px;color:#475569"> MB</span>';
    if (m.heap) {
      set('mon-heap-sub', m.heap.pct + '% des Heaps');
      set('mon-heap-detail', m.heap.usedMb + ' / ' + m.heap.totalMb + ' MB (' + m.heap.pct + '%)');
      const heapBar = document.getElementById('mon-heap-bar');
  ...
  // ─── Disk ────────────────────────────────────────────────────────────────────
  if (m.disk) {
    document.getElementById('mon-disk-val').innerHTML = m.disk.usedGb + '<span style="font-size:16px;color:#475569"> GB</span>';
    set('mon-disk-sub', 'von ' + m.disk.totalGb + ' GB');
    set('mon-disk-detail', m.disk.usedGb + ' / ' + m.disk.totalGb + ' GB (' + m.disk.pct + '%)');
    set('mon-disk-free', m.disk.freeGb + ' GB frei · ' + (100 - m.disk.pct).toFixed(0) + '% verfügbar');
    const diskBar = document.getElementById('mon-disk-bar');
    if (diskBar) {
      diskBar.style.width = m.disk.pct + '%';
      diskBar.style.background = m.disk.pct > 80 ? '#f87171' : '#f59e0b';
    }
  }
